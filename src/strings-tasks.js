...
function removeFirstOccurrences(str, value) {
  ...
  if (pos >= 0) {
    res = str.slice(0, pos) + str.slice(-(str.length - (pos + value.length)));
  }
  return res;
}
...
function findLongestWord(sentence) {
  ...
  arr.sort((el1, el2) => el2.length - el1.length);
  return arr[0];
}
...
function encodeToRot13(str) {
  const noCode = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const code = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    const pos = noCode.indexOf(str[i]);
    if (pos < 0) {
      res += str[i];
    } else {
      res += code[pos];
